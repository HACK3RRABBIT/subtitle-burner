...
; subtitleburner_gui/ is the native PySide6 app gui.py imports - Linux's
; gui_webview.py doesn't use it, but this .iss only ever builds the Windows
; installer, so it's unconditionally bundled here.
Source: "..\subtitleburner_gui\*"; DestDir: "{app}\subtitleburner_gui"; Flags: ignoreversion recursesubdirs createallsubdirs; Excludes: "__pycache__"
...
; The main GUI is now native PySide6, not a browser window - this shortcut is
; for anyone who still wants the Next.js web UI (e.g. to use it from another
; device on the LAN). launcher.py's console prompts (input() etc.) mean this
; needs a visible console, unlike the main windowless GUI shortcut above.
Name: "{group}\Subtitle Burner (Web UI)"; Filename: "{app}\python\python.exe"; Parameters: """{app}\launcher.py"""; WorkingDir: "{app}"; IconFilename: "{app}\assets\icon.ico"
